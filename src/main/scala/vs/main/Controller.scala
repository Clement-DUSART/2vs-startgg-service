...
class Controller[F[_]: Async: Functor](startGGCaller: StartGGCaller[F]) {
  ...
  val getEventsIdsEndPoint
      : Endpoint[Unit, (String, String), String, Seq[Event], Any] = endpoint
    .get
    .in("eventIds")
    .in(query[String]("tournament"))
    .in(auth.bearer[String]())
  ...
  val getEventsEndPoint
      : Endpoint[Unit, (String, String, String), String, Seq[BracketSet], Any] =
    endpoint
      .get
      .in("events")
